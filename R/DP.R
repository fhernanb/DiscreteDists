...
#'
#' @example examples/examples_DP.R
#'
#' @importFrom gamlss.dist checklink
#' @importFrom gamlss rqres.plot
#' @export
DP <- function(mu.link="log", sigma.link="log") {

  mstats <- checklink("mu.link", "DP",
                      substitute(mu.link), c("log"))
  dstats <- checklink("sigma.link", "DP",
                      substitute(sigma.link), c("log"))

  structure(list(family=c("DP", "discrete-Perks"),
                 ...
                 d2ldm2 = function(y, mu, sigma) {
                   dm   <- gamlss::numeric.deriv(dDP(y, mu, sigma, log=TRUE),
                                                 theta="mu",
                                                 delta=0.00001)
                 ...
                 d2ldmdd = function(y, mu, sigma) {
                   dm   <- gamlss::numeric.deriv(dDP(y, mu, sigma, log=TRUE),
                                                 ...
                                                 delta=0.00001)
                   dldm <- as.vector(attr(dm, "gradient"))
                   dd   <- gamlss::numeric.deriv(dDP(y, mu, sigma, log=TRUE),
                                                 theta="sigma",
                                                 delta=0.00001)
                 ...
                 G.dev.incr = function(y, mu, sigma, pw = 1, ...) -2*dDP(y, mu, sigma, log=TRUE),
                 rqres      = expression(rqres(pfun="pDP", type="Discrete",
                                               ymin = 0, y = y, mu = mu, sigma = sigma)),

                 mu.initial    = expression(mu    <- rep(estim_mu_sigma_DP(y)[1], length(y)) ),
                 sigma.initial = expression(sigma <- rep(estim_mu_sigma_DP(y)[2], length(y)) ),
...
#' estim_mu_sigma_DP
#'
#' This function generates initial values for DP distribution.
#'
#' @param y vector with the random sample
#' @examples
#' y <- rDP(n=100, mu=0.1, sigma=0.1)
#' estim_mu_sigma_DP(y=y)
#' @importFrom stats optim
#' @export
estim_mu_sigma_DP <- function(y) {
  mod <- optim(par=c(0, 0),
               fn=logLik_DP,
               method="Nelder-Mead",
...
#' logLik_DP
#'
#' This is an auxiliar function to obtain the logLik for DP.
#'
#' @param logparam vector with the values for mu sigma
#' @param x vector with the data
#' @examples
#' y <- rDP(n=100, mu=0.5, sigma=0.5)
#' logLik_DP(logparam=c(0, 0), x=y)
#' @importFrom stats optim
#' @export
logLik_DP <- function(logparam=c(0, 0), x){
  return(sum(dDP(x,
                 mu    = exp(logparam[1]),
