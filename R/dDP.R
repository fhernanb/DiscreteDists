...
#' function and random generation for the Perks, DP(), distribution
...
#'
#' @return
#' \code{dDP} gives the density, \code{pDP} gives the distribution
#' function, \code{qDP} gives the quantile function, \code{rDP}
#' generates random deviates.
#'
#' @example examples/examples_dDP.R
#'
#' @export
#'
dDP <- function(x, mu=0.5, sigma=0.5, log=FALSE){
...
#' @export
#' @rdname dDP
pDP <- function(q, mu=0.5, sigma=0.5, lower.tail=TRUE, log.p=FALSE){
  if (any(sigma <= 0))  stop("parameter sigma has to be positive!")
  if (any(mu <= 0))     stop("parameter mu has to be positive!")
...
#' @export
#' @rdname dDP
rDP <- function(n, mu=0.5, sigma=0.5) {
  if (any(sigma <= 0))  stop("parameter sigma has to be positive!")
  if (any(mu <= 0))     stop("parameter mu has to be positive!")
...
#' @export
#' @rdname dDP
qDP <- function(p, mu=0.5, sigma=0.5, lower.tail=TRUE, log.p=FALSE){
  if (any(sigma <= 0))  stop("parameter sigma has to be positive!")
  if (any(mu <= 0))     stop("parameter mu has to be positive!")
